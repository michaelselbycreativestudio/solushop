<?php 
	if (isset($_POST['submit'])) {
		...
		$sql = "SELECT * from neglmanagement where Username = '$usernameForm'";
		...
		if ($result->num_rows > 0) {
		       ...
		       //check if you have a matching record;   
		   		if ($usernameForm==$row["Username"] and $passwordForm == $row["PIN"]) {
		   			...
		   			$ManagerFName = $row["FName"];
		   			$ManagerLName = $row["LName"];
		   			$userRank = $row["Rank"];
		   			$ManagerID = $row["ID"];
		   			$matchFound = 1;
		   		}
...
<form method="POST" action="#">
	<div style="text-align:center;">
	  ...
	  <header style="font-family: 'Quicksand', sans-serif;"><img src="logo.png" width="200px" height="80px"><br>Eagle Express Portal</header>
	  <?php 
	  	if (isset($error)) {
