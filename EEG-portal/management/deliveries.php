...
<?php $DateTime = date("Y-m-d H:i:s"); 
    ...
    if (isset($_POST["MarkedAsDelivered"])) {
        ...
        $sql = "UPDATE order_items SET State='3' where Order_ID='$Order_ID' and Product_ID='$Order_Item_ID'";
        if ($conn->query($sql)) {
        ...
        //cr cash
        //credit cash
        $sql = "INSERT INTO `account_transactions` (`Transaction_ID`, `Transaction_Type`, `Transaction_Entity_ID`, `Transaction_Amount`, `Transaction_Description`, `Transaction_Date`) VALUES (NULL, 'Cr', '1', '$AmountToDebitSupplier', 'Payment for $Order_Item_ID-$Order_ID to Supplier, $Supplier_ID ($SupplierName)', '$DateTime');";
        $conn->query($sql);

        //debit supplier
        $sql = "INSERT INTO `account_transactions` (`Transaction_ID`, `Transaction_Type`, `Transaction_Entity_ID`, `Transaction_Amount`, `Transaction_Description`, `Transaction_Date`) VALUES (NULL, 'Dr', '3', '$AmountToDebitSupplier', 'Payment for $Order_Item_ID-$Order_ID to Supplier, $Supplier_ID ($SupplierName) from Cash', '$DateTime');";
        $conn->query($sql);

        //debit revenue
        //cr cash
        //credit cash
        $sql = "INSERT INTO `account_transactions` (`Transaction_ID`, `Transaction_Type`, `Transaction_Entity_ID`, `Transaction_Amount`, `Transaction_Description`, `Transaction_Date`) VALUES (NULL, 'Cr', '1', '$AmountRoDebitRevenue', 'Payment to Revenue $Order_Item_ID-$Order_ID delivered successfully', '$DateTime');";
        $conn->query($sql);

        //debit revebue
        $sql = "INSERT INTO `account_transactions` (`Transaction_ID`, `Transaction_Type`, `Transaction_Entity_ID`, `Transaction_Amount`, `Transaction_Description`, `Transaction_Date`) VALUES (NULL, 'Dr', '5', '$AmountRoDebitRevenue', 'Payment received from Cash for $Order_Item_ID-$Order_ID delivered successfully', '$DateTime');";
        $conn->query($sql);
        ...
        if ($OrderCompleteFlag==0) {
           //update order state to ready for delivery
            $sql = "UPDATE orders SET Order_State='5' WHERE Order_ID='$Order_ID'";
            $conn->query($sql);
        }
                            ...
                            <form method="POST" action="#" enctype="multipart/form-data" >
                                ...
                                if ($result->num_rows > 0) {
                                    ...
                                    while ($row = $result->fetch_assoc()) {
                                        ...
                                        $OrderItemSP[$ProductCount] = $row["SP"];
