<?php 
    ...
    if (isset($_POST["AddAddress"])) {
        ...
        if(!isset($ErrorMessage)){
            $sql = "INSERT INTO `customer_address` (`ID`, `Customer_ID`, `Region`, `Town`, `Address`) VALUES (NULL, \"$Customer_ID\", \"$Region \", \"$City\", \"$FinalAddress\")";
            if ($conn->query($sql)) {
                $sql = "SELECT * FROM customers WHERE Customer_ID='$Customer_ID'";
                $result = $conn->query($sql);
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        if ($row["Default_Address"]=='none') {
                            $UpdateDefaultAddress="Yes";
                        }
                    }
                }

                if ($UpdateDefaultAddress=="Yes") {
                    $sql = "SELECT MAX(ID) as ID FROM customer_address WHERE Customer_ID='$Customer_ID'";
                    $result = $conn->query($sql);
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            $JustAddedAddress = $row['ID'];
                        }
                    }

                    $sql = "UPDATE customers SET Default_Address='$JustAddedAddress' where Customer_ID='$Customer_ID'";
                    $conn->query($sql);
                    
                    }

                $SuccessMessage = "Address added successfully, $Customer_FName";
            }
            else{
                                ...
                                <div class="entry-content">
                                    ...
                                    <div style="width:75%; float:right" class="id-main">
                                       <h3>Add Address</h3>
                                                     ...
                                                     <input type="text" value="Select a Town / City and this will be updated automatically."  placeholder="" id="Region" name="Region" class="input-text " readonly/>
                                                          </p>
                                                    ...
                                                    <select type="text" value=""  placeholder="" id="City" name="City" class="input-text " onchange="update_Region()">
                                                    ...
                                                    <p id="billing_address_1_field" class="form-row form-row form-row-wide address-field validate-required"><label class="" for="billing_address_1">Address <abbr title="required" class="required">*</abbr></label><input type="text" value="" placeholder="Street address" id="StreetAddress" name="StreetAddress" class="input-text "></p>

                                                    <p id="billing_address_2_field" class="form-row form-row form-row-wide address-field"><input type="text" value=""  placeholder="Apartment, suite, unit etc. (optional)" id="AppartmentOrHouseNumber" name="AppartmentOrHouseNumber" class="input-text "></p>

                                                    <div style="text-align: center;">
                                                    <input type="submit" class="button" name="AddAddress" value="Save Address" />
                                                    </div>
