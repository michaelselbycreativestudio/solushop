...
<?php
    ...
    if (isset($_POST["MarkedAsPickedUp"])) {
        ...
        try{
            ...
            $change_state_query =  "UPDATE order_items SET State='2' where Order_ID='$Order_ID' and Product_ID='$Order_Item_ID'";
            $stmt = $db->prepare($change_state_query);
            //Execute
            $stmt->execute();
            if ($stmt->execute()) 
            {
                $SuccessMessage = "Order item $Order_Item_ID-$Order_ID marked as picked up and ready for delivery";
                //select order item details
                $order = "SELECT *, order_items.Quantity as OrderItemQuantity from order_items, products where order_items.Product_ID = products.ID and Order_ID='$Order_ID'";
                ...
                $order_result = $stmt->fetchAll(PDO::FETCH_OBJ);   
                    ...
                    for($i=0; $i<sizeof($order_result); $i++) {
                        $DescriptionOfItem = $order_result[$i]->DisplayLine;
                                $Quantity = $order_result[$i]->OrderItemQuantity;
                             }
                        }

                $order2 = "SELECT * from orders, customers where orders.Customer_ID = customers.Customer_ID and Order_ID='$Order_ID'";
                $stmt = $db->prepare($order2);
                $stmt->execute();
                $order2_result = $stmt->fetchAll(PDO::FETCH_OBJ);
                
                if (sizeof($order2_result)>0) {
                    for($i=0; $i<sizeof($order2_result); $i++) {
                        $OrderID = $order2_result[$i]->Order_ID;
                        $Customer_Name = $order2_result[$i]->FName." ".$order2_result[$i]->LName;
                        $Fname = $order2_result[$i]->FName;
                        $Lname = $order2_result[$i]->LName;
                        $Phone = $order2_result[$i]->Phone;
                        $Email = $order2_result[$i]->Email;
                    }
    ...
    //check if all the items are ready then change the order state
        ...
        if ($ReadyForDeliveryFlag==0) {
           //update order state to ready for delivery
            $update_order_state = "UPDATE orders SET Order_State='3' WHERE Order_ID='$Order_ID'";
            $stmt = $db->prepare($update_order_state);
            $stmt->execute();
        }
    }catch(PDOException $e){
        echo '{"error": {"text": '.$e->getMessage().'}';}
...
<html>
    <head>
        ...
        <!-- Morris chart -->
        <link href="../../as/sets/management/css/morris/morris.css" rel="stylesheet" type="text/css" />
        <!-- jvectormap -->
        ...
        <!-- Bootstr../../assets/managementap Color Picker -->
        <link href="css/colorpicker/bootstrap-colorpicker.min.css" rel="stylesheet"/
        <!-- Daterange picker -->
        <link href="../../assets/management/css/daterangepicker/daterangepicker-bs3.css" rel="stylesheet" type="text/css" />
        <!-- bootstrap wysihtml5 - text editor -->
        <link href="../../assets/managementcss/bootstrap-wysihtml5/bootstrap3-wysihtml5.min.css" rel="stylesheet" type="text/css" />
        <!-- Theme style -->
                            ...
                            <form method="POST" action="#" enctype="multipart/form-data" >
                                ...
                                <div class=\"box box-solid box-info\" >
                                    ...
                                    try{
                                        ...
                                        $db = new db(); //inititate db object
                                        $db = $db->connect();
                                          $address_query = "SELECT * FROM orders, order_items, order_items_state, products, suppliers WHERE order_items.state = order_items_state.ID and orders.Order_ID = order_items.Order_ID and order_items.Product_ID = products.ID and products.Supplier_ID = suppliers.ID and order_items.State = '1' ORDER BY Supplier_ID";
                                ...
                                if (sizeof($address_result)>0) {
                                    ...
                                    for($i=0; $i<sizeof($address_result); $i++) {
                                        $OrderItemID[$ProductCount] = $address_result[$i]->Order_ID."-".$address_result[$i]->Product_ID;

                                        $OrderItemDescription[$ProductCount] = $address_result[$i]->DisplayLine;

                                        $OrderItemRP[$ProductCount] = $address_result[$i]->RP;
                                        $SupplierName[$ProductCount] = $address_result[$i]->Name;
                                        $TotalDue += $OrderItemRP[$ProductCount];
                                         echo 
                                            "<tr>
                                                <td>".$OrderItemID[$ProductCount]."</td>    
                                                <td>".$OrderItemDescription[$ProductCount]."</td> 
                                                <td>".$address_result[$i]->Description."</td>    
                                        ...
                                        if ($address_result[$i]->Supplier_ID!=$PreviousSupplierID) {
                                            $SupplierIDCount++;
                                        }
                                        $ProductCount++;
                                    }
