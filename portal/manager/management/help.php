                                ...
                                <div class="box-body table-responsive no-padding">
                                            ...
                                            <?php
                                              ...
                                              if (isset($_POST['submit_table_search'])) {
                                                  ...
                                                  $stmt->execute();
                                                  $cus_select_results = $stmt->fetchAll(PDO::FETCH_OBJ);
                                              }else{
                                                    $cus_select="SELECT count(*) as count FROM customers";
                                                    $stmt = $db->prepare($cus_select);
                                                    $stmt->execute();
                                                    $cus_select_results = $stmt->fetchAll(PDO::FETCH_OBJ);
                                              }
                                            }catch(PDOException $e){echo '{"error": {"text": '.$e->getMessage().'}';}
                                            if (sizeof($cus_select_results) > 0) {
                                                  for($i=0; $i<sizeof($cus_select_results); $i++) {
                                                      $rec_count = $cus_select_results[$i]->count;
                                                  }
                                              }
                                             ...
                                             $select_cus = "SELECT *, customers.Customer_ID as CID  FROM customers";
                                             $stmt = $db->prepare($select_cus);
                                            }
                                            $stmt->execute();
                                            $select_cus_results = $stmt->fetchAll(PDO::FETCH_OBJ);

                                        }catch(PDOException $e){
                                            echo '{"error": {"text": '.$e->getMessage().'}';}

                                            if (sizeof($select_cus_results) > 0) {
                                              $Count=0;
                                              ...
                                              echo "<div class='row'>";
                                              for($i=0; $i< sizeof($select_cus_results); $i++){

                                                echo 
                                                    "<tr>
                                                                <td>".$select_cus_results[$i]->CID."</td>
                                                                <td>".$select_cus_results[$i]->FName." ".$select_cus_results[$i]->LName."</td>";
                                                                if ($select_cus_results[$i]->EmailVerified==0) {
                                                                    echo "<td>".$select_cus_results[$i]->Email."</td>";
                                                                }elseif ($select_cus_results[$i]->EmailVerified==1) {
                                                                    echo "<td>".$select_cus_results[$i]->Email."&nbsp;<img src='images/verified.png' style='margin-bottom: 5px; width:15px; height:15px;'></td>";
                                                                }
                                                                if ($select_cus_results[$i]->PhoneVerified==0) {
                                                                    echo "<td>".$select_cus_results[$i]->Phone."</td>";
                                                                }elseif ($select_cus_results[$i]->PhoneVerified==1) {
                                                                    echo "<td>".$select_cus_results[$i]->Phone."&nbsp;<img src='images/verified.png' style='margin-bottom: 5px; width:15px; height:15px;'></td>";
                                                                }
                                                                echo "<td><a href='editcustomerdetails.php?id=".$select_cus_results[$i]->Customer_ID."'><button class='btn btn-info btn-sm'>Edit Details</button></a>&nbsp;&nbsp;<a href='viewcustomerorders.php?id=".$select_cus_results[$i]->Customer_ID."'><button class='btn btn-success btn-sm'>View Orders</button></a></td>
                                                            </tr>";
