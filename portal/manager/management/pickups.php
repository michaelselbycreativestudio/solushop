...
<?php
    ...
    if (isset($_POST["MarkedAsPickedUp"])) {
        ...
        if ($stmt->execute()) {
            ...
            $order = "SELECT *, order_items.Quantity as OrderItemQuantity from order_items, products where order_items.Product_ID = products.ID and Order_ID='$Order_ID' and Product_ID='$Order_Item_ID'";
        ...
        try{
            $check_ready = "SELECT * FROM order_items WHERE Order_ID= :Order_ID";
            $stmt = $db->prepare($check_ready);
            $stmt->bindParam(':Order_ID', $Order_ID);
           $stmt->execute();
...
<html>
    <head>
        ...
        <!-- jvectormap -->
        <link href="../../as/sets/management/css/jvectormap/jquery-jvectormap-1.2.2.css" rel="stylesheet" type="text/css" />
                            ...
                            try{
                                ...
                                if (sizeof($order_result)>0) {
                                    ...
                                    echo "<div class=\"box-body table-responsive no-padding\">
                                     ...
                                     for($i=0; $i<sizeof($order_result); $i++) {
                                        $OrderItemID[$ProductCount] = $order_result[$i]->Order_ID."-".$order_result[$i]->Product_ID;
                                        $OrderItemDescription[$ProductCount] = $order_result[$i]->DisplayLine;
                                        $OrderItemRP[$ProductCount] = $order_result[$i]->RP;
                                        $OrderItemQuantity[$ProductCount] = $order_result[$i]->OrderItemQuantity;
                                        $SupplierName[$ProductCount] = $order_result[$i]->Name;
                                        $TotalDue += $OrderItemRP[$ProductCount];
                                         echo 
                                            "<tr>
                                                ...
                                                <td>".$order_result[$i]->Description."</td>    
                                        ...
                                        if ($order_result[$i]->Supplier_ID!=$PreviousSupplierID) {
                                            $SupplierIDCount++;
                                        }
                                        $ProductCount++;
                                    }
                                            ...
                                            <tr>
                                                ...
                                                <td width=\"50%\">";
                                                    if ($PickUpGuide == "Available") {
                                                        ...
                                                        echo "<a href=\"../../Reports-And-Summaries/Pick-Ups/$FileName.pdf\" download=\"$FileName.pdf\"> <input type='submit' style='background-color:green; border-color:green;' class='btn btn-warning btn-sm' name='DownloadPickUpGuide' value='Download'/></a>";
                                                    }else{
