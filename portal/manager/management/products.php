                                ...
                                <div class="box-body table-responsive no-padding">
                                            ...
                                            <?php
                                              ...
                                              try{
                                                    ...
                                                    $db = $db->connect();
                                                    if(isset($_POST['submit_table_search'])){
                                                        $select_products="SELECT count(*) as count FROM products, product_categories, product_pictures where product_pictures.ProductID = products.ID and products.Category = product_categories.CategoryCode and (DisplayLine LIKE '%$SearchValue%' OR CategoryDescription LIKE '%$SearchValue%' OR HighlightedFeatures LIKE '%$SearchValue%')  and PicturePath LIKE '%a%'";
                                                        $stmt = $db->prepare($select_products);

                                                      }else{
                                                      $select_products = "SELECT count(*) as count FROM products, product_categories, product_pictures where product_pictures.ProductID = products.ID and products.Category = product_categories.CategoryCode and PicturePath LIKE '%a%' ";
                                                      $stmt = $db->prepare($select_products);
                                                        }
                                                        $stmt->execute();
                                            ...
                                            for($i=0; $i<sizeof($product_result); $i++) {
                                             ...
                                             try{
                                              if (isset($_POST['submit_table_search'])) {
                                                $search_products = "SELECT * FROM products, product_categories, product_pictures where product_pictures.ProductID = products.ID and products.Category = product_categories.CategoryCode and (DisplayLine LIKE '%$SearchValue%' OR CategoryDescription LIKE '%$SearchValue%' OR HighlightedFeatures LIKE '%$SearchValue%') and PicturePath LIKE '%a%' ";
                                                $stmt = $db->prepare($search_products);
                                            }else{
                                             $search_products = "SELECT * FROM products, product_categories, product_pictures where product_pictures.ProductID = products.ID and products.Category = product_categories.CategoryCode and PicturePath LIKE '%a%' ORDER BY ID ASC LIMIT $offset, $rec_limit";
                                             $stmt = $db->prepare($search_products);
                                            }
                                            $stmt->execute();
                                            ...
                                            if (sizeof($search_result)>0) {
                                              ...
                                              for($i=0; $i<sizeof($search_result); $i++) {
                                                $imageURL ="../../../assets/images/products/thumbnails/".$search_result[$i]->ID."a.png";
                                                $StockID = $search_result[$i]->ID;

                                                echo 
                                                    "<tr>
                                                                <td>".$search_result[$i]->ID."</td>
                                                                <td>".$search_result[$i]->DisplayLine."</td>
                                                                <td><div class='img' style=\"background-image:url($imageURL); margin-right: 20px;\"></div></td>
                                                                <td>".$search_result[$i]->Views."</td>
                                                                <td><a href='viewproduct.php?id=".$search_result[$i]->ID."'><button class='btn btn-warning btn-sm'>Details</button></td>
                                                            </tr>";
