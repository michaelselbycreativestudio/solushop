                         ...
                         if (sizeof($trans_results) > 0) {
                             for($i=0; i<sizeof($trans_results);$i++){
                                if ($row["Transaction_Type"] == "Cr") {
                                    $SupplierCreditFromCash += $trans_results[$i]->Transaction_Amount;
                                }elseif ($trans_results[$i]->Transaction_Type == "Dr") {
                                    $SupplierDebitToCash += $trans_results[$i]->Transaction_Amount;
                                }
                        ...
                        <section class="col-lg-7 connectedSortable">
                            <?php 
                                ...
                                try{
                                    $db = new db(); //inititate db object
                                    $db = $db->connect();
                                    $select_ac_trans = "SELECT * FROM account_transactions, account_entities WHERE account_transactions.Transaction_Entity_ID = account_entities.Entity_ID and Transaction_Description LIKE '%$SupplierID%' ORDER BY Transaction_ID DESC";
                                    $stmt = $db->prepare($select_ac_trans);
                                    $stmt->bindParam(':SupplierID',$SupplierID);
                                    $stmt->execute();
                                    $ac_trans_result = $stmt->fetchAll(PDO::FETCH_OBJ);
                                    $db = null;
                                }catch(PDOException $e){
                                    echo '{"error": {"text": '.$e->getMessage().'}';
                                }
                                
                                if (sizeof($ac_trans_result) > 0) {
                                    ...
                                    for($i==0; $i<sizeof($ac_trans_result);$i++) {
                                         echo 
                                            "<tr>
                                                <td>".$ac_trans_result[$i]->Transaction_ID."</td>
                                                <td>".$ac_trans_result[$i]->Entity_Description."</td>";
                                                if (strpos($ac_trans_result[$i]->Transaction_Description, "PAY-IN") !== false) {
                                                        echo "<td><img src='images/MoneyMoveExternalIn.png' style=' width:25px; height:25px;'></td>";
                                                }elseif (strpos($ac_trans_result[$i]->Transaction_Description, "PAY-OUT") !== false) {
                                                    echo "<td><img src='images/MoneyMoveExternalIn.png' style='width:25px; height:25px;'></td>";
                                                }elseif ($ac_trans_result[$i]->Transaction_Type =="Cr") {
                                                   echo "<td><img src='images/MoneyMoveInternalOut.png' style='width:25px; height:25px;'></td>";
                                                }elseif ($ac_trans_result[$i]->Transaction_Type =="Dr") {
                                                   echo "<td><img src='images/MoneyMoveInternalIn.png' style='width:25px; height:25px;'></td>";
                                                }
                                                echo"
                                                <td>".$ac_trans_result[$i]->Transaction_Amount."</td> 
                                                <td>".$ac_trans_result[$i]->Transaction_Description."</td> 
                                                <td>".$ac_trans_result[$i]->Transaction_Date."</td> 
                                            </tr>";
                                    }
                                }else{
