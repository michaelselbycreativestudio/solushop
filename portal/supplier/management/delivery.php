                            ...
                            <div class="box">
                                ...
                                <div class="box-body table-responsive no-padding">
                                            ...
                                            <?php
                                              ...
                                              if (isset($_POST['submit_table_search'])) {
                                                  $select_cus_count = "SELECT count(*) as count  FROM customers LEFT JOIN customer_address ON customers.Customer_ID = customer_address.Customer_ID where Region LIKE '%:SearchValue%' OR Town LIKE '%:SearchValue%' OR Address LIKE '%:SearchValue%' OR FName LIKE '%:SearchValue%' OR LName LIKE '%:SearchValue%' OR Email LIKE '%:SearchValue%' OR EmailVerified LIKE '%:SearchValue%' OR Phone LIKE '%:SearchValue%' OR PhoneVerified LIKE '%:SearchValue%' OR ActivationCode LIKE '%:SearchValue%'";
                                                  $stmt = $db->prepare($select_cus_count);
                                                  $stmt->bindParam(':SearchValue',$SearchValue);
                                                  $stmt->execute();
                                                  $cus_result = $stmt->fetchAll(PDO::FETCH_OBJ);
                                              }else{
                                                    $select_cus_count="SELECT count(*) as count FROM customers";
                                                    $stmt = $db->prepare($select_cus_count);
                                                  $stmt->execute();
                                                  $cus_result = $stmt->fetchAll(PDO::FETCH_OBJ);
                                              }
                                            ...
                                            if (sizeof($cus_result) > 0) {
                                                for($i=0; $i<sizeof($cus_result);$i++){
                                                        $rec_count = $cus_result[$i]->count;
                                                    }
                                                }
                                             ...
                                             try{
                                              if (isset($_POST['submit_table_search'])) {
                                                $select_cus_id = "SELECT *, customers.Customer_ID as CID FROM customers LEFT JOIN customer_address ON customers.Customer_ID = customer_address.Customer_ID where Region LIKE '%:SearchValue%' OR Town LIKE '%:SearchValue%' OR FName LIKE '%:SearchValue%' OR LName LIKE '%:SearchValue%' OR Email LIKE '%:SearchValue%' OR EmailVerified LIKE '%:SearchValue%' OR Phone LIKE '%:SearchValue%' OR PhoneVerified LIKE '%:SearchValue%' OR ActivationCode LIKE '%:SearchValue%' ";
