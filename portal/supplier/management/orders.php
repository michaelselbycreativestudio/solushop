                                   ...
                                   <div class="box-tools">
                                    ...
                                    <?php 
                                        if (isset($_POST['submit_table_search'])) {
                                             $TableSearch = $SearchValue = mysqli_real_escape_string($conn, $_POST['table_search']);
                                ...
                                <div class="box-body table-responsive no-padding">
                                            ...
                                            <?php
                                              ...
                                              try{
                                                ...
                                                $db = $db->connect();
                                                $order_count = "SELECT count(*) as count FROM orders, customers where customers.Customer_ID = orders.Order_ID";
                                                $stmt = $db->prepare($order_count);
                                                $stmt->execute();
                                                $order_count_result = $stmt->fetchAll(PDO::FETCH_OBJ);
                                            }catch(PDOException $e){
                                                echo '{"error": {"text": '.$e->getMessage().'}';}
                                            if (sizeof($order_count_result) > 0) {
                                                for($i=0; $i<sizeof($order_count_result);$i++){
                                                    $rec_count = $order_count_result[$i]->count;
                                                }
                                            }
                                             ...
                                             try{
                                              if (isset($_POST['submit_table_search'])) {
                                                $order_cus = "SELECT * FROM orders, customers where customers.Customer_ID = orders.Customer_ID and (FName LIKE '%:SearchValue%' OR LName LIKE '%:SearchValue%' OR Email LIKE '%:SearchValue%' OR EmailVerified LIKE '%:SearchValue%' OR Phone LIKE '%:SearchValue%' OR PhoneVerified LIKE '%:SearchValue%' OR ActivationCode LIKE '%:SearchValue%')";
                                                $stmt = $db->prepare($order_cus);
                                                $stmt->bindParam(':SearchValue',$SearchValue);
                                                $stmt->execute();
                                                $order_cus_result = $stmt->fetchAll(PDO::FETCH_OBJ);
                                            }else{
                                             $order_cus = "SELECT * FROM orders, customers where customers.Customer_ID = orders.Customer_ID";
                                             $stmt = $db->prepare($order_cus);
                                             $stmt->execute();
                                             $order_cus_result = $stmt->fetchAll(PDO::FETCH_OBJ);
                                            }
                                            $db =null;
                                              ...
                                              for($i=0; $i<sizeof($order_cus_result);$i++){
                                                if ($order_cus_result[$i]->Order_State==1) {
                                                    $OrderState = "<span class='label label-info'>Pending Confirmation</span>";
                                                }elseif ($order_cus_result[$i]->Order_State==2) {
                                                    $OrderState = "<span class='label label-warning'>Confirmed, being processed</span>";
                                                }elseif ($order_cus_result[$i]->Order_State==3) {
                                                    $OrderState = "<span style='background-color:#0275d8' class='label label-info'>Ready for Delivery</span>";
                                                }elseif ($order_cus_result[$i]->Order_State==4) {
                                                    $OrderState = "<span style='background-color:#32CD32' class='label label-info'>Delivering</span>";
                                                }elseif ($order_cus_result[$i]->Order_State==5) {
                                                    $OrderState = "<span style='background-color:green' class='label label-success'>Completed</span>";
                                                }elseif ($order_cus_result[$i]->Order_State==6) {
                                                    $OrderState = "<span style='' class='label label-danger'>Cancelled</span>";
                                                }
                                                echo 
                                                    "<tr>
                                                                <td>".$order_cus_result[$i]->Order_ID."</td>
                                                                <td>".$order_cus_result[$i]->FName." ".$order_cus_result[$i]->LName."</td>
                                                                <td>".$order_cus_result[$i]->Email."</td>
                                                                <td>".$order_cus_result[$i]->Phone."</td>
                                                                <td>".$order_cus_result[$i]->NEGLDescription."</td>
                                                                <td><a href='vieworderdetails.php?id=".$order_cus_result[$i]->Order_ID."'><button class='btn btn-success btn-sm'>View Details</button></a></td>
                                                            </tr>";
                                          
                                            }
                                        } else {
                                            echo "<br><br><br><h4 style='text-align:center;'> No photos found.</h4><br><br><br><br><br><br>";
                                        }
