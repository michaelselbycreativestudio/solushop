                            ...
                            <div class="box">
                                ...
                                <div class="box-body table-responsive no-padding">
                                        ...
                                        try{
                                            ...
                                            $db = $db->connect();
                                            if(isset($_POST['submit_table_search'])){
                                            $select_prod_count="SELECT count(*) as count FROM products, product_categories, product_pictures where products.Supplier_ID = :SupplierID  and product_pictures.ProductID = products.ID and products.Category = product_categories.CategoryCode and (DisplayLine LIKE '%:SearchValue%' OR CategoryDescription LIKE '%:SearchValue%' OR HighlightedFeatures LIKE '%:SearchValue%')  and PicturePath LIKE '%a%'";
                                            $stmt = $db->prepare($select_prod_count);
                                            $stmt->bindParam(':SupplierID',$SupplierID);
                                            $stmt->bindParam(':SearchValue',$SearchValue);
                                            $stmt->execute();
                                            $prod_count_result = $stmt->fetchAll(PDO::FETCH_OBJ);

                                            }else{
                                                $select_prod_count = "SELECT count(*) as count FROM products, product_categories, product_pictures where products.Supplier_ID = :SupplierID and  product_pictures.ProductID = products.ID and products.Category = product_categories.CategoryCode and PicturePath LIKE '%a%' ";
                                                $stmt = $db->prepare($select_prod_count);
                                                $stmt->bindParam(':SupplierID',$SupplierID);
                                                $stmt->execute();
                                                $prod_count_result = $stmt->fetchAll(PDO::FETCH_OBJ);
                                            ...
                                            if (sizeof($prod_count_result) > 0) {
                                                for($i=0; $i<sizeof($prod_count_result);$i++){
                                                        $rec_count = $prod_count_result[$i]->count;
                                                    }
                                                }
                                ...
                                echo '{"error": {"text": '.$e->getMessage().'}';}
                                        ...
                                        try{
                                            ...
                                            if (isset($_POST['submit_table_search'])) {
                                            $select_products = "SELECT * FROM products, product_categories, product_pictures where products.Supplier_ID = :SupplierID  and product_pictures.ProductID = products.ID and products.Category = product_categories.CategoryCode and (DisplayLine LIKE '%:SearchValue%' OR CategoryDescription LIKE '%:SearchValue%' OR HighlightedFeatures LIKE '%:SearchValue%') and PicturePath LIKE '%a%' ";
                                            ...
                                            $products_result = $stmt->fetchAll(PDO::FETCH_OBJ);
                                             ...
                                             $select_products = "SELECT * FROM products, product_categories, product_pictures where products.Supplier_ID = :SupplierID  and product_pictures.ProductID = products.ID and products.Category = product_categories.CategoryCode and PicturePath LIKE '%a%' ORDER BY ID ASC LIMIT $offset, $rec_limit";
                                             $stmt = $db->prepare($select_products);
                                             $stmt->bindParam(':SupplierID',$SupplierID);
                                             $stmt->execute();
                                             $products_result = $stmt->fetchAll(PDO::FETCH_OBJ);
                                            ...
                                            if (sizeof($products_result) > 0) {
                                              ...
                                              for($i=0; $i<sizeof($products_result);$i++){
                                                $imageURL ="../../assets/images/products/thumbnails/".$products_result[$i]->ID."a.png";
                                                $StockID = $products_result[$i]->ID;

                                                echo 
                                                    "<tr>
                                                                <td>".$products_result[$i]->ID."</td>
                                                                <td>".$products_result[$i]->DisplayLine."</td>
                                                                <td><div class='img' style=\"background-image:url($imageURL); margin-right: 20px;\"></div></td>
                                                                <td>".$products_result[$i]->Views."</td>
                                                                <td><a href='viewproduct.php?id=".$products_result[$i]->ID."'><button class='btn btn-warning btn-sm'>Details</button></td>
                                                            </tr>";
