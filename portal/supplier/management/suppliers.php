                            ...
                            <div class="box">
                                ...
                                <div class="box-body table-responsive no-padding">
                                            ...
                                            <?php
                                              ...
                                              if (isset($_POST['submit_table_search'])) {
                                                  $count_cus = "SELECT count(*) as count  FROM customers LEFT JOIN customer_address ON customers.Customer_ID = customer_address.Customer_ID where Region LIKE '%:SearchValue%' OR Town LIKE '%:SearchValue%' OR Address LIKE '%:SearchValue%' OR FName LIKE '%:SearchValue%' OR LName LIKE '%:SearchValue%' OR Email LIKE '%:SearchValue%' OR EmailVerified LIKE '%:SearchValue%' OR Phone LIKE '%:SearchValue%' OR PhoneVerified LIKE '%:SearchValue%' OR ActivationCode LIKE '%:SearchValue%'";
                                                  $stmt = $db->prepare($count_cus);
                                                  $stmt->bindParam(':SearchValue',$SearchValue);
                                                  $stmt->execute();
                                                  $cus_count_result = $stmt->fetchAll(PDO::FETCH_OBJ);
                                              
                                              }else{
                                                    $count_cus="SELECT count(*) as count FROM suppliers";
                                                    $stmt = $db->prepare($count_cus);
                                                    $stmt->execute();
                                                    $cus_count_result = $stmt->fetchAll(PDO::FETCH_OBJ);
                                              }
                                                ...
                                                if (sizeof($cus_count_result) > 0) {
                                                    for($i=0; $i<sizeof($cus_count_result);$i++){
                                                        $rec_count = $cus_count_result[$i]->count;
                                                    }
                                                }
                                             ...
                                             try{
                                              if (isset($_POST['submit_table_search'])) {
                                                $cus_id = "SELECT *, customers.Customer_ID as CID FROM customers LEFT JOIN customer_address ON customers.Customer_ID = customer_address.Customer_ID where Region LIKE '%:SearchValue%' OR Town LIKE '%:SearchValue%' OR Address LIKE '%:SearchValue%' OR FName LIKE '%:SearchValue%' OR LName LIKE '%:SearchValue%' OR Email LIKE '%:SearchValue%' OR EmailVerified LIKE '%:SearchValue%' OR Phone LIKE '%:SearchValue%' OR PhoneVerified LIKE '%:SearchValue%' OR ActivationCode LIKE '%:SearchValue%' ";
                                                $stmt = $db->prepare($cus_id);
                                                $stmt->bindParam(':SearchValue',$SearchValue);
                                                $stmt->execute();
                                                $cus_id_result = $stmt->fetchAll(PDO::FETCH_OBJ);
                                            }else{
                                             $cus_id = "SELECT * FROM suppliers";
                                             $stmt = $db->prepare($cus_id);
                                             $stmt->execute();
                                             $cus_id_result = $stmt->fetchAll(PDO::FETCH_OBJ);
                                            }
                                         ...
                                         if (sizeof($cus_id_result) > 0) {
                                            ...
                                            for($i=0; $i<sizeof($cus_id_result);$i++){
                                                ...
                                                echo 
                                                    "<tr>
                                                                <td>".$cus_id_result[$i]->ID."</td>
                                                                <td>".$cus_id_result[$i]->Username."</td>
                                                                <td>".$cus_id_result[$i]->Name."</td>
                                                                <td>".$cus_id_result[$i]->Email."</td>
                                                                <td>".$cus_id_result[$i]->Phone."</td>
                                                                <td>".$cus_id_result[$i]->AltPhone."</td>
                                                                <td><a href='ciewsupplierdetails.php?id=".$cus_id_result[$i]->ID."'><button class='btn btn-success btn-sm'>View Details</button></a></td>
                                                            </tr>";
