...
<head>
	...
	<title>Shop in Ghana's Most Trusted Online Store</title>
	...
	<div class="wrapper">
        ...
        <?php 
            ...
            try{
                $db = new db();
                $db = $db->connect();
                $sql_product_query = "SELECT *, products.ID as productsID from products, suppliers WHERE products.Supplier_ID = suppliers.ID AND (products.ID = '".$ProductToDisplay[0]."'";

                for ($i=1; $i < sizeof($ProductToDisplay); $i++) { 
                    $sql_product_query .= " OR products.ID = '".$ProductToDisplay[$i]."'";
                }
                $sql_product_query .= ") ORDER BY FIELD(products.ID ";
                for ($i=1; $i < sizeof($ProductToDisplay); $i++) { 
                    $sql_product_query .= ",".$ProductToDisplay[$i]." ";
                }
                $sql_product_query .= ")";
                $stmt = $db->prepare($sql_product_query);
                $stmt->execute();
		                    ...
		                    <div class="row">
		                        ...
		                        <div class="col-md-12">
		                            <div class="shop-product-img mb-30 img-full">
		                                <img src="assets/img/offer/11.jpg" alt="">
		                            </div>
		                        </div>
		                        <!--Shop Product Image Start-->
		                    </div>
                                                ...
                                                <div class=\"col-md-3 col-sm-3 item-col2\">
                                                    <div class=\"single-product\">
                                                        <div class=\"product-img\">
                                                            <a href=\"product.php?PID=".$display_product[$i]->productsID."\">
                                                                <img class=\"first-img\" src=\"assets/img/products/thumbnails/".$display_product[$i]->productsID."a.jpg\" alt=\"\">
                                                            </a>
                                                            <ul class=\"product-action\">
                                                                <li><a href=\"shop.php?add_to_wishlist=".$display_product[$i]->productsID."\" data-toggle=\"tooltip\" title=\"Add to Wishlist\"><i class=\"ion-android-favorite-outline\"></i></a></li>
                                                                <li><a href=\"shop.php?add_to_cart=".$display_product[$i]->productsID."\" data-toggle=\"tooltip\" title=\"Add to Cart\"><i class=\"ion-ios-cart\"></i></a></li>
                                                            </ul>
                                                        </div>
                                                        <div class=\"product-content\">
                                                            <h2 style='margin-bottom:5px;'><a href=\"single-product.php\">".$display_product[$i]->DisplayLine."</a></h2>
                                                            <h5>by <a style='color:#f68b1e' href=\"vendor.php?VID=".$display_product[$i]->ID."\"><b>".$display_product[$i]->Username."</b></a></h5>
